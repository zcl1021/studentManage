...
<body>
	...
	<a href="/jygl/recruitManage?action=create&rid=${recruit.rid}">修改</a>
	<a href="/jygl/recruitManage?action=delete&rid=${recruit.rid}">删除</a>
	   ...
	   <div class="pagediv">
              <span>
                 <a href="/jygl/recruitManage?action=edit&page=1&sql=${doPage.sql }">首页</a>&nbsp;&nbsp;
                 <%--如果当前页面不是第一页，显示上一页选项 --%>
                 <c:if test="${doPage.nowPage>1 }">
                    <a href="/jygl/recruitManage?action=edit&page=${doPage.nowPage-1 }&sql=${doPage.sql}">上一页</a>
                    &nbsp;&nbsp;
                 </c:if>
                 <%--如果当前页面不是最后一页，显示下一页选项 --%>
                 <c:if test="${doPage.nowPage<doPage.totalPage}">
                    <a href="/jygl/recruitManage?action=edit&page=${doPage.nowPage+1 }&sql=${doPage.sql}">下一页</a>
                     &nbsp;&nbsp;
                 </c:if>
                   <%--显示末页 --%>
                 
                    <a href="/jygl/recruitManage?action=edit&page=${doPage.totalPage }&sql=${doPage.sql}">末页</a>
                      &nbsp;&nbsp;
